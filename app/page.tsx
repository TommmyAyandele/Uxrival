...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  :root {
    ...
    --text: #f0f0f0; --text-muted: #a0a0b0; --text-dim: #6a6a80;
  ...
  .modal-content { background: #0f0f18; }
  ...
  .nav { display: flex; align-items: center; justify-content: space-between; padding: 22px 0; border-bottom: 1px solid #1a1a2a; position: sticky; top: 0; background: rgba(10,10,18,0.95); backdrop-filter: blur(12px); z-index: 200; }
  ...
  .modal-header { padding: 24px 24px 0; flex-shrink: 0; background: #13131e; }
  .modal-close { position: absolute; top: 20px; right: 20px; width: 36px; height: 36px; background: #1a1a22; border: 1px solid #2a2a3a; border-radius: 8px; color: var(--text-muted); font-size: 20px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.15s; line-height: 1; }
  .modal-close:hover { color: var(--text); border-color: var(--accent); }
  .modal-scroll { overflow: auto; padding: 24px; flex: 1; min-height: 0; }
  .modal-actions { display: flex; gap: 8px; padding: 20px 24px 24px; flex-shrink: 0; border-top: 1px solid #2a2a3a; background: #13131e; }
  ...
  input[type="text"]:focus, textarea:focus { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(232,255,71,0.06); outline: none; }
  ...
  .btn-secondary { background: #1a1a22; border: 1px solid #2a2a3a; color: var(--text); border-radius: 8px; padding: 9px 16px; font-family: var(--font-d); font-size: 12px; font-weight: 600; cursor: pointer; transition: all 0.15s; display: flex; align-items: center; gap: 6px; white-space: nowrap; }
  ...
  .score-card { flex: 1; min-width: 100px; max-width: 180px; padding: 16px; background: #13131e; border: 1px solid #2a2a3a; border-radius: var(--radius); }
  ...
  input:focus, textarea:focus, button:focus, .btn-primary:focus, .btn-secondary:focus, .dropdown-trigger:focus, .theme-toggle-option:focus, .view-toggle-btn:focus, .nav-link:focus, .modal-close:focus, .mobile-menu-item:focus {
...
export default function UXRival() {
  ...
  script.onload = () => {
    ...
    let y = 20;
    const category = industry === "custom" ? customCategory : industry;

    // Header
    doc.setFontSize(20); doc.setFont('helvetica', 'bold');
    doc.text(category, 20, y); y += 8;
    doc.setFontSize(10); doc.setFont('helvetica', 'normal'); doc.setTextColor(136,136,136);
    doc.text(`Deep Teardown · ${reportData.comps.join(', ')}`, 20, y); y += 10;

    // Headline
    doc.setFontSize(13); doc.setFont('helvetica', 'bold'); doc.setTextColor(0,0,0);
    const headlineLines = doc.splitTextToSize(reportData.headline, pageWidth - 40);
    doc.text(headlineLines, 20, y); y += headlineLines.length * 7 + 6;

    // Scores
    doc.setFontSize(10); doc.setFont('helvetica', 'normal');
    Object.entries(reportData.scores).forEach(([name, score]: any) => {
      doc.setTextColor(0,0,0); doc.text(`${name}: `, 20, y);
      doc.setTextColor(score >= 80 ? 22 : score >= 60 ? 202 : 220, score >= 80 ? 163 : score >= 60 ? 138 : 38, score >= 80 ? 74 : score >= 60 ? 4 : 38);
      doc.text(`${score}/100`, 50, y); y += 7;
    }); y += 4;

    // Sections
    reportData.secs.forEach((sec: any) => {
      if (y > 260) { doc.addPage(); y = 20; }
      doc.setFillColor(240,240,240); doc.rect(20, y-4, pageWidth-40, 8, 'F');
      doc.setFontSize(9); doc.setFont('helvetica', 'bold'); doc.setTextColor(100,100,100);
      doc.text(sec.cat.toUpperCase(), 22, y); y += 8;

      sec.rows.forEach((row: any) => {
        if (y > 260) { doc.addPage(); y = 20; }
        doc.setFontSize(10); doc.setFont('helvetica', 'bold'); doc.setTextColor(0,0,0);
        doc.text(row.dim, 20, y); y += 6;
        Object.entries(row.sc).forEach(([comp, rating]: any) => {
          doc.setFont('helvetica', 'normal'); doc.setFontSize(9); doc.setTextColor(80,80,80);
          doc.text(`${comp}: ${rating.r}`, 24, y); y += 5;
        });
        if (row.ins) {
          doc.setTextColor(100,100,100);
          const ins = doc.splitTextToSize(`Insight: ${row.ins}`, pageWidth - 48);
          doc.text(ins, 24, y); y += ins.length * 5;
        }
        if (row.rec) {
          doc.setTextColor(91,33,182);
          const rec = doc.splitTextToSize(`Your Move: ${row.rec}`, pageWidth - 48);
          doc.text(rec, 24, y); y += rec.length * 5 + 3;
        }
      });
    });
    ...
    y += 4;
    doc.setFillColor(255,251,235); doc.rect(20, y-4, pageWidth-40, 14, 'F');
    doc.setFontSize(9); doc.setFont('helvetica', 'bold'); doc.setTextColor(146,64,14);
    doc.text('OPPORTUNITY', 22, y); y += 6;
    doc.setFont('helvetica', 'normal'); doc.setTextColor(120,53,15);
    const opp = doc.splitTextToSize(reportData.opp, pageWidth - 44);
    doc.text(opp, 22, y); y += opp.length * 5 + 8;

    // Footer
    doc.setFontSize(8); doc.setTextColor(170,170,170);
    doc.text('Generated by UXRival.com — Free AI UX Analysis', pageWidth/2, 285, { align: 'center' });

    doc.save(`uxrival-${category.replace(/\s+/g,'-').toLowerCase()}.pdf`);
  };
