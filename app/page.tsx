  ...
  printWindow.document.write(`
    ...
    <head>
      ...
      <style>
        ...
        .score-card .number { font-size: 32px; font-weight: 900; }
        table { width: 100%; border-collapse: collapse; margin-bottom: 24px; }
        th { background: #f5f5f5; font-size: 11px; text-transform: uppercase; color: #666; padding: 10px 12px; text-align: left; }
        td { padding: 10px 12px; font-size: 13px; border-bottom: 1px solid #f0f0f0; vertical-align: top; }
        tr:nth-child(even) { background: #fafafa; }
        .section-row td { background: #f0f0f0; font-size: 11px; text-transform: uppercase; color: #888; font-weight: 600; }
        .excellent { color: #16a34a; font-weight: 700; }
        .good { color: #65a30d; font-weight: 700; }
        .average { color: #ca8a04; font-weight: 700; }
        .poor { color: #ea580c; font-weight: 700; }
        .weak { color: #dc2626; font-weight: 700; }
        .your-move { color: #5b21b6; }
        .key-insight { color: #666; }
        .gap { background: #fffbeb; border-left: 3px solid #ca8a04; padding: 12px 16px; margin-top: 16px; }
        .gap-label { font-size: 10px; font-weight: 700; color: #92400e; text-transform: uppercase; margin-bottom: 4px; }
    ...
    <body>
      ...
      <table>
        <thead>
          <tr>
            <th>Dimension</th>
            ${reportData.comps.map((c: string) => `<th>${c}</th>`).join('')}
            <th>Key Insight</th>
            <th>Your Move</th>
          </tr>
        </thead>
        <tbody>
          ${reportData.secs.map((sec: any) => `
            <tr class="section-row"><td colspan="${reportData.comps.length + 3}">${sec.cat}</td></tr>
            ${sec.rows.map((row: any) => `
              <tr>
                <td>${row.dim}</td>
                ${reportData.comps.map((c: string) => {
                  const rating = row.sc?.[c]?.r || "";
                  return `<td class="${rating.toLowerCase()}">${rating}</td>`;
                }).join('')}
                <td class="key-insight">${row.ins || ""}</td>
                <td class="your-move">${row.rec || ""}</td>
              </tr>
            `).join('')}
          `).join('')}
        </tbody>
      ...
      ${reportData.opp ? `
        <div class="gap">
          <div class="gap-label">Opportunity</div>
          <div>${reportData.opp}</div>
        </div>
      ` : ""}
      <div class="footer">Generated by UXRival.com — Free AI UX Analysis</div>
    </body>
    </html>
  `);
